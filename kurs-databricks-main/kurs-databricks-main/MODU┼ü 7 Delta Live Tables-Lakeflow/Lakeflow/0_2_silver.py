...
# MAGIC
# MAGIC Funkcja dekorowana `@dp.table` tworzy w potoku **Lakeflow** tabelę warstwy *silver* o nazwie `silver.<schema>.<param_source_name>`.
# MAGIC
# MAGIC 1. `@dp.expect` Expectations do kontroli jakości
...
# 1: Tworzę widok
@dp.view
@dp.expect("valid_rating", "rating_value >= 0 AND rating_value <= 5")
@dp.expect("non_null_item_weight_or_drop", "item_weight IS NOT NULL")
@dp.expect("non_null_hash", "hash_value IS NOT NULL")
@dp.expect("valid_inserted_at", "inserted_at IS NOT NULL")

def silver_books():
    df_bronze = dp.read_stream(f"bronze.{schema}.{param_source_name}")
